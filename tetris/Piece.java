...
public class Piece {
	...
	private List<TPoint> pointsList;
...
//		Arrays.sort(body, (TPoint a, TPoint b) -> (a.x - b.x < 0 ? -1 : (b.x - a.x < 0 ? 1 : a.y - b.y)));
		...
		for(int i = 0; i < width; i++) {
			...
			for(int j = 0; j < points.length; j++) {
				if(points[j].x == i) skirt[i] = Math.min(skirt[i], points[j].y);
			}
		}
	}
...
//		Arrays.sort(other.body, (TPoint a, TPoint b) -> (a.x - b.x < 0 ? -1 : (b.x - a.x < 0 ? 1 : a.y - b.y)));
		...
		for(TPoint p : other.body) if(!pointsList.contains(p)) return false;
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		Piece cp = root;
		cp.next = root.computeNextRotation();
		while(!root.equals(cp.next)) {
			cp = cp.next;
			cp.next = cp.computeNextRotation();
		}
		return cp.next = root;
	}
